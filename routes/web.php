...
Route::middleware(['auth'])->group(function () {
    ...
    Route::post('/beli', function (Request $request) {
        ...
        $finalAmount = max(0, ($originalAmount - $discount) + $kodeUnik);
        ...
        if ($reseller) {
            $recipients = $recipients->merge([$reseller]);
        }
        ...
        if ($setting && $setting->apikey && $setting->codeqr) {
            ...
            $amountEncoded  = urlencode($finalAmount);
            $codeqrEncoded  = urlencode($setting->codeqr);

            $url = "https://apiku-fafa-main.vercel.app/api/orkut/createpayment?apikey={$apikey}&amount={$amountEncoded}&codeqr={$codeqrEncoded}";
    ...
    Route::get('/mutasi-manual/{id}', function ($id) {
        ...
        try {
            ...
            foreach ($data['result'] as $mutasi) {
                ...
                if ($transaksi->status === 'pending' && $transaksi->amount == $nominal) {
